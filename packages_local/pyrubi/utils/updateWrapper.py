class UpdateWrapper:
    ...
    @property
    def time(self) -> str:
        return self.update["chat_updates"][0]["chat"]["last_message"].get("time")
    ...
    @property
    def action(self) -> str:
        return self.update["message_updates"][0].get("action")
    ...
    def metadata(self) -> str:
        return self.update["message_updates"][0]["message"].get("metadata")
    ...
    @property
    def text(self) -> str:
        return self.update["message_updates"][0]["message"].get("text")
    ...
    def reply(self, text:str) -> dict:
        return self.methods.send_text(object_guid=self.object_guid, text=text, message_id=self.message_id)
    
    def seen(self) -> dict:
        return self.methods.seen_chats(seen_list={self.object_guid: self.message_id})
    
    def reaction(self, reaction:int) -> dict:
        return self.methods.reaction_message(object_guid=self.object_guid, message_id=self.message_id, reaction=reaction)
    
    def delete(self, deleteForAll:bool=True) -> dict:
        return self.methods.delete_messages(object_guid=self.object_guid, message_ids=[self.message_id], delete_for_all=deleteForAll)
    
    def pin(self) -> dict:
        return self.methods.pin_message(object_guid=self.object_guid, message_id=self.message_id)
    
    def forward(self, to_object_guid:str) -> dict:
        return self.methods.forward_messages(object_guid=self.object_guid, message_ids=[self.message_id], toObject_guid=to_object_guid)
    
    def ban(self) -> dict:
        return self.methods.ban_member(object_guid=self.object_guid, member_guid=self.author_guid)
    
    def check_join(self, object_guid:str) -> bool:
        return self.methods.check_join(object_guid=object_guid, user_guid=self.author_guid)
    
    def download(self, save:bool=False, save_as:str=None) -> dict:
        return self.methods.download(save=save, save_as=save_as, file_inline=self.file_inline)